...
{
    var strokeColor;
    if(element.current) { //是否是当前
        strokeColor = CURRENT_COLOR; //当前颜色
    } else if(element.completed) {// 是否完成
...
function _drawFlow(flow){
	...
	if (!((historyModelId && historyModelId != "") || (modelId && modelId != ""))) {
...
function _drawArrowHead(line, strokeColor)
{
	...
	arrowHead.attr("fill", strokeColor);

	arrowHead.attr("stroke-width", SEQUENCEFLOW_STROKE);
	arrowHead.attr("stroke", strokeColor);
