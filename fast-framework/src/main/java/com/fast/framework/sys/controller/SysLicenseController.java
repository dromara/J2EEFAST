...
import com.fast.common.core.license.LicenseCheck;
import com.fast.common.core.license.LicenseVerify;
import com.fast.common.core.license.creat.LicenseCreator;
import com.fast.common.core.license.creat.LicenseCreatorParam;
...
public class SysLicenseController extends AbstractController {
    ...
    @Value("${license.privateAlias: j2eefast}")
    private String privateAlias;
    @Value("${license.keyPass: j2eefast.com@zhouzhou}")
    private String keyPass;
    @Value("${license.storePass: j2eefast.com}")
    private String storePass;
    @Value("${license.privateKeysStorePath: license/privateKeys.keystore}")
    ...
    @RepeatSubmit
    @ResponseBody
    public R uploadLic(@RequestParam("licfile") MultipartFile file){
        try{
        ...
        catch (Exception e)
        {
            ...
            return R.error("许可证书无效,请确认无误在上传!");
        }
    }
    ...
    @ResponseBody
    public R checkverifyNoUnique(String sn, String verifyNo){
        ...
        try{
           ...
           if(HexUtil.encodeHexStr(SM4.encryptData_ECB(clear,ConfigConstant.FAST_VERIFY_KEY))
               ...
               return R.error("验证码不正确,请核对!");
           }
        }catch (Exception e){
            return R.error("机器码或验证码不正确,请核对!");
        }
    }
    ...
    @RepeatSubmit
    @ResponseBody
    public R creatLicense(String sn, String verifyNo, String ip){
        ...
        try{
            ...
            if(ToolUtil.isNotEmpty(ip)){
                ...
                for(String ipdrr: ips){
                    if(!ToolUtil.isBoolIp(ipdrr)){
                        return R.error("输入的IP地址不合法!");
                    }else{
                        ipList.add(ipdrr);
                    }
                }
            }
            R r = checkverifyNoUnique(sn, verifyNo);
            if(checkverifyNoUnique(sn, verifyNo).get("code").equals("00000")){
                ...
                return R.ok().put("path",hexstr).put("checkCode",checkCode);
            }else{
