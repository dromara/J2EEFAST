...
import com.fast.framework.sys.mapper.SysNoticeMapper;
...
//        sysNotice = this.getOne(new QueryWrapper<SysNoticeEntity>().eq("status","0").eq("notice_type","1").
//                eq("notice_level","2").orderByAsc("start_time").last("limit 1"));
//        if(ToolUtil.isNotEmpty(sysNotice)){
//            long longTime;
//            listNotice = new ArrayList<>(1);
//            listNotice.add(sysNotice);
//            longTime = DateUtil.between(new Date(),sysNotice.getEndTime(), DateUnit.SECOND);
